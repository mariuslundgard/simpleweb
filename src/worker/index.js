...
function create (config: Config) {
  const { logger } = config
  const redis = createRedisClient(config.redis, 'worker')
  ...
  async function handleTask (id) {
    ...
    try {
      ...
      const taskHandler = taskHandlers[startedTask.type]

      if (!taskHandler) {
        throw new Error(`Unknown task: ${startedTask.type}`)
      }

      await taskHandler(config, redis, db, startedTask)
    } catch (err) {
    ...
    publisher.publish('task:completed', startedTask.type)

    if (startedTask.type === 'flushall') {
      return
    }
